...
if (isset($_GET['busca_nome']) && !empty($_GET['busca_nome'])) {
    ...
    $lista = $dalTurma->SelectByNome($termo);
} else {
    $termo = "";
    $lista = $dalTurma->Select();
}
...
<body class="blue lighten-5">
    <div class="container">
        <h1 style="text-align: center; color: #1976d2; margin-top: 40px;">Listar Turmas</h1>

        <div class="row yellow lighten-3 black-text" style="margin-top: 20px; padding: 15px; border-radius: 5px;">
            <form action="lstTurma.php" method="get" class="col s12">
        ...
        <div class="row" style="margin-top: 20px;">
            <div class="col s12">
                <a href="frminsTurma.php" class="btn waves-effect waves-light green">
                    <i class="material-icons left">add</i>Nova Turma
                </a>
            </div>
        </div>
        ...
        <table class="striped responsive-table hover" style="margin-top: 20px;">
            ...
            <?php
                foreach ($lista as $turma) {
                    ...
                    echo '<td>' . htmlspecialchars($turma->getNome()) . '</td>';
                    echo '<td>' . htmlspecialchars($turma->getHorario()) . '</td>';
                    echo '<td>' . htmlspecialchars($turma->getDiaSemana()) . '</td>';
                    echo '<td>' . htmlspecialchars($turma->getNivel()) . '</td>';
                    echo '<td>';
                    echo '<a class="btn-floating btn-small waves-effect waves-light blue" href="frmedtTurma.php?id=' . $turma->getId() . '"><i class="material-icons">edit</i></a>';
                    echo '<a class="btn-floating btn-small waves-effect waves-light orange" href="frmdetTurma.php?id=' . $turma->getId() . '"><i class="material-icons">visibility</i></a>';
                    echo '<a class="btn-floating btn-small waves-effect waves-light red" href="javascript:if(confirm(\'Tem certeza?\')){window.location=\'opremTurma.php?id=' . $turma->getId() . '\'}"><i class="material-icons">delete</i></a>';
                    echo '</td>';
                    echo '</tr>';
                }
            ?>
